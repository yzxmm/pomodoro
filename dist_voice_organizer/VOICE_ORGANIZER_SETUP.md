...
首次点击“识别”按钮时，软件会自动下载 Whisper 的模型文件（默认为 `small` 模型，约 461MB），相比 `base` 拥有更高的中英日韩文识别准确率。

**【推荐】一键预下载模型**：
为了避免首次使用时等待，您也可以运行我们提供的脚本提前下载模型：
1.  进入 `tools` 文件夹。
2.  双击运行 `download_models.bat`。
3.  按提示输入要下载的模型名称，例如：`small` 或 `small medium`。
4.  等待脚本显示 "All selected models are ready!" 或 "All required models are ready!"。
